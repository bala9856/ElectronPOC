...
ipcMain.handle('create-user', (event, user) => {
  return new Promise((resolve, reject) => {
    db.run('INSERT INTO users (name, email, age, active, category, skills, avatar, gender, birthdate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', 
      [user.name, user.email, user.age, user.active, user.category, user.skills, user.avatar, user.gender, user.birthdate], function(err) {
      if (err) reject(err);
...
ipcMain.handle('update-user', (event, user) => {
  return new Promise((resolve, reject) => {
    db.run('UPDATE users SET name = ?, email = ?, age = ?, active = ?, category = ?, skills = ?, avatar = ?, gender = ?, birthdate = ? WHERE id = ?',
      [user.name, user.email, user.age, user.active, user.category, user.skills, user.avatar, user.gender, user.birthdate, user.id], (err) => {
      if (err) reject(err);
      else resolve(user);
    });
  });
