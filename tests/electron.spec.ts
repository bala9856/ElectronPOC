...
import { test } from "./fixtures/launchFixture";
//const fs = require('fs');
...
test.describe("User Managment", () => {
  test("Verify Home Page", { tag: ["@positive", "@regression"] }, async ({ mainWindow }) => {
  ...
  test("Update User", { tag: ["@positive", "@regression"] }, async ({ mainWindow }) => {
    const homePage = new HomePage(mainWindow);
    const name = testData.userplayWright.username;
  ...
  test("Delete User", { tag: ["@positive", "@regression"] }, async ({ mainWindow }) => {
    const homePage = new HomePage(mainWindow);
    await homePage.printTitle();
  ...
  test("Negative check on Age", { tag: ["@negative", "@regression"] }, async ({ mainWindow }) => {
    const homePage = new HomePage(mainWindow);
    const name = testData.userplayWright.username;
    const eMail = testData.userplayWright.Email;
  ...
  test("Verify Luminometer", { tag: ["@negative", "@regression"] }, async ({ mainWindow }) => {
  ...
  test("Luminometer Connection", { tag: ["@positive", "@regression"] }, async ({ mainWindow }) => {
    const homePage = new HomePage(mainWindow);
